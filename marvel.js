const heroes = [
    ...
    },
    {
      ...
      "info": "Супергероиня российского происхождения..."
    },
    {
      "name": "Дэдпул",
      "universe": "Marvel Comics",
      ...
      "info": "Как и Росомаха из Людей Икс, Дэдпул был подвергнут опытам по программе «Оружие Икс»..."
    }
  ];
...
heroes.forEach(hero => {
    ...
    ratingInput.min = 0;
    ratingInput.max = 10;
    ratingInput.value = localStorage.getItem(hero.name) || 0; 
    ratingInput.addEventListener('change', () => {
        localStorage.setItem(hero.name, ratingInput.value); 
    });

    const rating = document.createElement('div');
    rating.classList.add('hero-rating');
